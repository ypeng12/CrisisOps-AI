...
export const RecommendationPanel: React.FC<Props> = ({ actions, onActionStateChange }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-panel border border-border rounded-lg p-4 overflow-y-auto custom-scrollbar">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="text-lg font-semibold tracking-wide">Recommended Actions</h2>
      </div>
        ...
        {actions.map(action => (
          <div key={action.id} className="bg-background border border-border rounded p-3 relative overflow-hidden">
            <div className="flex justify-between items-start mb-2">
              ...
              <div className="absolute top-3 right-3">
                {action.state !== 'Pending' ? (
                   <StatusBadge text={action.state} type="state" />
                ) : (
            ...
            <div className="flex justify-between items-center text-xs">
              <span className="text-accent font-mono">Conf: {(action.confidence * 100).toFixed(0)}%</span>
