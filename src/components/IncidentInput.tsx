...
export const IncidentInput: React.FC<Props> = ({ t, onAnalyze, onSimulateSecond, hasInitialReport }) => {
    ...
    <div className="flex flex-col h-full bg-panel border border-border rounded-lg p-4 overflow-y-auto custom-scrollbar">
      ...
      <div className="mt-4 pt-4 border-t border-border">
        <div className="bg-blue-950/20 border border-blue-900/30 rounded p-3">
          <h3 className="text-xs font-bold text-blue-400 uppercase mb-2">{t.whyNotChatbot}</h3>
          <ul className="text-xs text-gray-400 space-y-1">
            <li><span className="text-red-400 line-through mr-1">Chatbot:</span> {t.chatbotDesc}</li>
            <li><span className="text-green-400 font-bold mr-1">CrisisOps AI:</span> {t.crisisOpsDesc}</li>
          </ul>
        </div>
      </div>
    </div>
